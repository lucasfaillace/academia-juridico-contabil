...
export async function POST(request: Request) {
  const ip = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "local"; const now = Date.now(); const recent = (hits.get(ip) || []).filter((time) => now - time < 60 * 60 * 1000); if (recent.length >= 5) return NextResponse.json({ error: "Muitas tentativas" }, { status: 429 }); recent.push(now); hits.set(ip, recent);
