...
O Google Analytics 4 pode ser ativado em **Configurações → Google Analytics 4**. Informe o ID `G-...` e salve; a alteração fica no PostgreSQL e passa a valer nas páginas públicas sem novo build. A variável `NEXT_PUBLIC_GA_MEASUREMENT_ID` permanece opcional como fallback quando não houver configuração salva. O script do Google é carregado no início da navegação pública, nunca é carregado em `/admin`, `/admin/login` ou prévias administrativas e também permanece desativado nas páginas públicas enquanto houver uma sessão administrativa autenticada. O aviso de cookies é informativo e seu botão controla somente a exibição da mensagem.
